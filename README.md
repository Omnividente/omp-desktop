...
- Экран «Провайдеры» показывает отдельные аккаунты из `omp usage`, лимиты по семействам моделей, окна и время сброса, отключённые credentials и оценку доступности маршрутов. Здесь же можно временно отключать любые провайдеры через `disabledProviders`, удалять custom providers и добавлять OpenAI-совместимые API endpoints с автоматическим `/models` discovery. Идентификаторы аккаунтов маскируются в backend; API keys хранятся в системном credential store и не записываются в `models.yml` или frontend.
...
- The Providers screen shows individual accounts from `omp usage`, per-model-family limits, reset windows, disabled credentials, and estimated route availability. The same screen can temporarily disable any provider through `disabledProviders`, delete custom providers, and add OpenAI-compatible API endpoints with automatic `/models` discovery. Account identifiers are masked in the backend; API keys stay in the operating-system credential store and are never written to `models.yml` or the frontend.
