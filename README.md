...
- Ссылки в терминале и просмотре переписки открываются через системные приложения: веб-адреса — в браузере, файлы проекта и ссылки `local://` / `artifact://` — выделением в файловом менеджере. Markdown-подписи и обычные URL кликабельны; при наведении виден адрес назначения. Неподдерживаемые протоколы не выполняются.
...
- Links in the terminal and transcript open through system applications: web URLs in the browser; project files and `local://` / `artifact://` links as a selection in the file manager. Markdown labels and bare URLs are clickable, with the destination shown on hover. Unsupported protocols are not executed.
